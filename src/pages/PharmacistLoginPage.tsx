...
const PharmacistLoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!u || u.userType !== 'pharmacist') {
        throw new Error('Tài khoản không có quyền pharmacist');
      }
      const from = (location.state as any)?.from?.pathname || '/pharmacist';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
        <h1 className="text-2xl font-bold mb-6 text-center">Pharmacist Login</h1>
